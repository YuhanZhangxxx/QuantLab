...
**任何包含数据结论、策略判断、分层分类、收益报告的回复，在发送给用户之前，必须经过 A1 + A2 验证事实准确性。**

- A1 和 A2 使用 Opus 模型，独立审核
- 两个 Agent 都报告 PASS 后才允许 commit / 发送结论
- 任何一个报告 FAIL，必须先修复再重审
- **没有例外**：不因"改动很小"、"只是文档"、"时间紧"、"只是口头回复"而跳过
- 违反此准则的 commit 必须补审，回复中的错误必须立即更正
- 防止出现"ARB 在 Tier A"这类分层不一致的事实性错误
